...
{
    public class GetProizvodiPoKategorijiQueryHandler
        : IRequestHandler<GetProizvodiPoKategorijiQuery, IEnumerable<ProizvodDto>>
    {
        private readonly IUnitOfWork _unitOfWork;

        public GetProizvodiPoKategorijiQueryHandler(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }

        public Task<IEnumerable<ProizvodDto>> Handle(
            GetProizvodiPoKategorijiQuery request,
            CancellationToken cancellationToken)
        {
            var proizvodi = _unitOfWork.Proizvodi
                .GetSviPoKategoriji(request.KategorijaId)
