...
{
    public class GetAllProizvodiQueryHandler : IRequestHandler<GetAllProizvodiQuery, IEnumerable<ProizvodDto>>
    {
        ...
        public Task<IEnumerable<ProizvodDto>> Handle(GetAllProizvodiQuery request, CancellationToken cancellationToken)
        {
            var proizvodi = _unitOfWork.Proizvodi.GetAll();

            var rezultat = proizvodi.Select(p => MapirajProizvod(p)).ToList();
