...
{
    public class GetProizvodiPoMaxCeniQueryHandler
        : IRequestHandler<GetProizvodiPoMaxCeniQuery, IEnumerable<ProizvodDto>>
    {
        private readonly IUnitOfWork _unitOfWork;

        public GetProizvodiPoMaxCeniQueryHandler(IUnitOfWork unitOfWork)
        {
            _unitOfWork = unitOfWork;
        }

        public Task<IEnumerable<ProizvodDto>> Handle(
            GetProizvodiPoMaxCeniQuery request,
            CancellationToken cancellationToken)
        {
            var proizvodi = _unitOfWork.Proizvodi
                .GetSviSaMaxCenom(request.MaxCena)
